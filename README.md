...
| `docs/index.html` | Dashboard shell — HTML structure + CSS only; React app loaded from `_app.js` |
| `docs/_app.js` | **Pre-compiled** React/JSX app (auto-generated by `push.ps1` — do not edit directly) |
| `docs/.nojekyll` | Tells GitHub Pages to serve all files including `_app.js` (files starting with `_` are blocked by Jekyll by default) |
| `babel.config.json` | Babel config — compiles JSX using the classic `React.createElement` runtime (required since React is loaded from CDN, not npm) |
| `push.ps1` | One-command deploy: compiles JSX → pushes to Apps Script → redeploys web app → commits to GitHub |

---

## Dashboard Development

The React dashboard (`docs/index.html`) uses **pre-compiled JSX** rather than in-browser Babel transpilation. This was necessary because the dashboard grew beyond ~22,000 lines of JSX — large enough to cause Babel standalone to time out in the browser, resulting in a blank page.

### Architecture

```
docs/index.html       ← HTML shell + all CSS (edit this for UI changes)
      │
      └── <script src="_app.js">
                │
                └── docs/_app.js   ← compiled output (do NOT edit)
                         ▲
                         │  babel compile
                         │
              (JSX extracted from index.html at push time)
```

`push.ps1` automatically:
1. Extracts the `<script type="text/babel">` block from `index.html`
2. Compiles it to `docs/_app.js` using Babel CLI with the classic runtime
3. Pushes everything to Apps Script + GitHub

### Making dashboard changes

1. Edit `docs/index.html` — add your React components/JSX inside the `<script type="text/babel">` block as normal.
2. Run `push.ps1` — the compile step runs automatically.
3. GitHub Pages serves the updated `_app.js` within a few minutes.

> **Never edit `docs/_app.js` directly** — it is overwritten every time `push.ps1` runs.

### Blank page troubleshooting

| Symptom | Likely cause | Fix |
|---------|-------------|-----|
| Blank dark-blue page, no console errors | Babel timed out compiling — file too large | Run `push.ps1` to ensure `_app.js` is up to date |
| `_app.js` 404 on GitHub Pages | Jekyll blocking `_` files | Ensure `docs/.nojekyll` exists |
| `Uncaught ReferenceError: App is not defined` | `_app.js` using ES module `import` syntax | Check `babel.config.json` has `"runtime": "classic"` |
...
| Dashboard works locally but not on GitHub Pages | GitHub Pages hasn't redeployed yet | Wait 2–5 min, or check Actions tab for deploy status |
...
1. Compiles `docs/index.html` JSX → `docs/_app.js`
