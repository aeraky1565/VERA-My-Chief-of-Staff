...
function buildChatSystemPrompt_(context) {
  ...
  return (
    ...
    'ACTION:create_project|{project name}|{task 1}~{task 2}~{task 3}   (each task may be "Task|Priority|Phase")\n' +
    ...
    '- For create_project: before generating the plan, ask 2–4 targeted clarifying questions to understand scope, timeline, and constraints. Wait for the answers before emitting the ACTION line. Only skip questions if Ahmed has already provided enough context, or explicitly says "just create it" / "go ahead". Once you have the details, ' +
    // The planning judgement itself lives in Projects.js, shared with the
    // dashboard's drafting endpoint, so the two paths cannot drift apart.
    PROJECT_PLAN_GUIDANCE_ +
    ' Separate tasks with ~ and append |Priority|Phase to each task. Example: "Book the venue|High|Logistics".\n' +
