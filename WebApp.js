...
/**
 * GET get_luggage_profiles — returns all active bag profiles
 * Optional param: owner (ahmed|victoria|shared) to filter
 */
function webGetLuggageProfiles_(e) {
  ...
  var profiles = data
    .filter(function(r) {
      if (String(r[8] || '').trim().toUpperCase() === 'FALSE') return false; // Active = FALSE
...
/**
 * GET save_luggage_profile — create or update a bag profile
 * Params: id? (omit to create), owner, name, type, capacityL, maxWeightKg?, dimensionsCm?, notes?
 */
...
/**
 * GET delete_luggage_profile — soft-delete (sets Active = FALSE)
 * Params: id
 */
function webDeleteLuggageProfile_(e) {
  ...
  for (var i = 0; i < ids.length; i++) {
    if (String(ids[i][0]).trim() === id) {
      sheet.getRange(i + 2, 9).setValue('FALSE'); // col I: Active
      return { ok: true };
    }
  }
  throw new Error('Profile not found: ' + id);
...
/**
 * GET get_trip_luggage — returns selected bag IDs + resolved profiles for a trip
 * Params: tripKey
 */
function webGetTripLuggage_(e) {
  ...
  // Read luggage JSON from TripMeta col 10 (index 9)
  var ss        = getSpreadsheet();
  ...
  // Resolve profile objects for selected IDs
...
/**
 * GET set_trip_luggage — saves selected bag IDs to TripMeta
 * Params: tripKey, bagIds (comma-separated or JSON array string)
 */
function webSetTripLuggage_(e) {
  ...
  // Find existing row and update luggage JSON (preserve other luggage data like allocations)
  ...
  // No existing row — append a minimal one
  var today = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  metaSheet.appendRow([tripKey, '', '', today, '', '', '', '', '', JSON.stringify({ bagIds: bagIds })]);
  return { ok: true };
}

/**
 * GET allocate_luggage — AI-powered bag allocation for a trip's packing list
 * Params: tripKey
 * Returns: { ok, assignments, recommendation, capacityByBag }
 */
